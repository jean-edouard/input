...
static int create_new_hidg(void)
{
  ...
  /* Creating the sysfs node along with the first hidg */
  if (id == 0)
...
{
	struct hidg_device_node *e, *n;

	platform_driver_unregister(&hidg_plat_driver);
	list_for_each_entry_safe(e, n, &hidg_devices, node) {
		platform_device_unregister(e->dev);
		list_del(&e->node);
		kfree(e);
	}
